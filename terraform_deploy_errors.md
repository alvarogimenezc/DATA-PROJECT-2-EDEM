...
Solo existía `terraform.tfvars.example`. `terraform plan` falla con:
...
Solo dos archivos tocados, **sin eliminar nada**. Los cambios son additive y reproducen el patrón ya presente para los demás recursos.
...
| Proyecto tras destroy | `gcloud pubsub topics list`, `gcloud run …`, `bq ls` | Vacío (solo SA default y 3 buckets preservados con datos) |
